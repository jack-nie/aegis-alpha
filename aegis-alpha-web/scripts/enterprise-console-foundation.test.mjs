...
assert.match(source, /\/admin\/audit-events/, "Audit log page should call the admin audit-events API.");
...
for (const action of ["dispatch", "pause", "resume", "cancel"]) {
  assert.ok(
    source.includes(`data-testid="workflow-run-${action}"`) || hasDynamicRunActionMarker,
    `Workflow run center should render ${action} control.`,
  );
}
