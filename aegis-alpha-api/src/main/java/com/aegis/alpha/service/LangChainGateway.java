...
public class LangChainGateway {
    ...
    public LangChainGateway(ObjectMapper objectMapper,
                            @Value("${aegis.langchain.enabled:false}") boolean enabled,
                            @Value("${aegis.langchain.engine-url:http://127.0.0.1:8787}") String engineUrl,
                            @Value("${aegis.langchain.provider:openai}") String provider,
                            @Value("${aegis.langchain.model:deepseek-v4-flash}") String defaultModel,
    ...
    public Map<String, Object> executeNode(AgentTemplate agent, Map<String, Object> state, Map<String, Object> node, String subject) {
        ...
        return localStructuredNode(agent, state, node, subject, "LangGraph disabled. Set AEGIS_ALPHA_LANGCHAIN_ENABLED=true to use the external engine.");
    }
